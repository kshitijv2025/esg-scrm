...
from src.connectors.mqtt_client import SmartMeterConsumer, _insert_metric
...
class TestSmartMeterConsumer:
    ...
    def test_on_message_parses_valid_payload(self):
        ...
        msg = MagicMock()
        msg.topic = "factory/test_factory/meter/meter_01/energy_kwh"
        ...
        assert consumer._batch[0]["cluster"] == "energy_kwh"
    ...
    def test_on_message_extracts_meter_id(self):
        consumer = SmartMeterConsumer()
        msg = MagicMock()
        msg.topic = "factory/f1/meter/meter_42/water_m3"
        msg.payload = b'{"value": 500, "unit": "m3"}'
        consumer.on_message(None, None, msg)
        assert consumer._batch[0]["meter_id"] == "meter_42"
        assert consumer._batch[0]["cluster"] == "water_m3"
