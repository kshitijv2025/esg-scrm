...
@pytest.fixture(autouse=True)
def _mock_live_integrations(monkeypatch):
    """Allow seeded data to flow through during tests.

    The production code checks _has_live_integrations() to decide whether
    to return real data (MQTT/SAP connected) or blank. Tests seed the DB
    with deterministic data, so we mock this to return True so tests can
    assert against the seeded values.
    """
    monkeypatch.setattr(
        "src.api.routes.dashboard._has_live_integrations", lambda: True
    )
