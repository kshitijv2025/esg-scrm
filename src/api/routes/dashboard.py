...
def _has_live_integrations() -> bool:
    """Return True if at least one real data integration is connected.

    Real integrations: MQTT broker (smart meters) or SAP Business One.
    Without these, the database only contains seed/demo data — show blank.
    """
    ...
    try:
        ...
        # MQTT not reachable — no live smart meter data
        pass
    else:
        return True  # MQTT is connected, data is live

    # Check SAP Business One — password must be non-empty
    sap_password = os.environ.get("SAP_B1_PASSWORD", "")
    if sap_password:
        return True  # SAP is configured, data may be live

    return False


def _build_metric_display(row, meta):
...
def emission_intensity(user: dict = Depends(require_auth)):
    ...
    Returns zeroed data when no real integrations are connected (no fake demo data).
    """
    if not _has_live_integrations():
        return {
            "org_id": user["org_id"],
            ...
            "emission_intensity_tco2_per_unit": 0,
            "period": "",
        }
    org_id = user["org_id"]
...
def trends(metric_type: str, user: dict = Depends(require_auth)):
    if not _has_live_integrations():
        return {"metric_type": metric_type, "data": [], "unit": ""}
    org_id = user["org_id"]
...
def operations_summary(user: dict = Depends(require_auth)):
    ...
    If no real integrations (MQTT, SAP) are connected, returns empty clusters
    so the UI shows a blank state instead of demo seed data.
    """
    if not _has_live_integrations():
        return {
            "clusters": {},
