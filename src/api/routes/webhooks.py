...
def create_webhook(
    ...
    # SSRF guard: reject URLs resolving to internal/reserved IPs at registration time
    if not _is_safe_url(payload.get("url", "")):
        raise HTTPException(
            status_code=400,
            detail="URL resolves to an internal or reserved IP address and is not allowed",
        )
...
def update_webhook(
    ...
    try:
        ...
        for field in ["name", "url", "is_active"]:
            if field in payload:
                # SSRF guard for URL updates
                if field == "url" and not _is_safe_url(payload[field]):
                    raise HTTPException(
                        status_code=400,
                        detail="URL resolves to an internal or reserved IP address",
                    )
